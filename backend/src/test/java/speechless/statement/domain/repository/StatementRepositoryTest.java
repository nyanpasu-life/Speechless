...
public class StatementRepositoryTest {
    ...
    public void StatementSaveTest() {
        ...
        // request 처리
        Statement statement = getStatement(statementRequest, memeber);
    ...
    public void statementsReadTest() {
        ...
        // given
        Page<Statement> list = repository.findAllByMember(memeber,
            PageRequest.of(0, 1));
        ...
        // request 처리
        Statement statement1 = getStatement(statementRequest1, memeber);
        Statement statement2 = getStatement(statementRequest2, memeber);

        repository.save(statement1);
        repository.save(statement2);

        // when
        list = repository.findAllByMember(memeber,
            PageRequest.of(0, 1));
    ...
    public void statementReadTest() {
        ...
        Statement result = repository.findByMemberIdAndId(statement.getMember().getId(),
                statement.getId())
    ...
    private Statement getStatement(StatementRequest request, Member member) {

        Statement statement = StatementMapper.INSTANCE
            .toEntity(request).toBuilder().member(member).build();
